...
<template>
  ...
  <!-- 坐标轴 -->
  <Coordinate id="coordinate" :elemPos="axis" />
  <!-- 服务区 -->
...
import ShowDetails from '../overviewRight/details/showDetails.vue'
import QualityInspectionDetails from '../overviewRight/details/qualityInspectionDetails.vue'
import { viteImages } from '@/utils'
import BlackBim from '@/utils/blackHole'
import { inject, onMounted, ref, onBeforeUnmount, computed, nextTick, watch, reactive } from 'vue'
...
const mapshow = (pos: number[], id: string, type: string) => {
  ...
  if (
    ...
    if (type == 'center') {
      ...
      ele.style.left = screenPos[0] - 48 + 'px'
      ele.style.bottom = screenPos[1] - 48 + 'px'
    }
    // 根据需求调视口远近，div进行隐藏
...
const RECameraMove = () => {
  ...
  if (axis.value.length) {
    mapshow(axis.value, 'coordinate', 'axis')
  }
  // 工序数据
...
onMounted(() => {
  ...
  //   [114.71720832721002, 30.1069962887021, 0]
  // ])[0]
  // console.log(pos)
...
watch(
  ...
  () => {
    nextTick(() => {
      axis.value = component.state.axis
      RECameraMove()
    })
  },
  { deep: true }
...
watch(
  ...
  () => {
    // console.log(newVal, '工序信息')
    const list = infoStore.gxinfo.list
    let show = true
    if (list.length) {
      list.map((e: GXINFO) => {
        if (show) {
          const x = monomer.filter((x) => e.unitWorkName.indexOf(x.name.replace(/号/g, '#')) !== -1)
          if (x.length) {
            show = false
...
watch(
  ...
  () => {
    // console.log(infoStore.zjinfo.list, '质检信息')
    const list = infoStore.zjinfo.list
    let show = true
    if (list.length) {
      list.map((e: ZJINFO) => {
        if (show) {
          const x = monomer.filter((x) => e.gcbw.indexOf(x.name.replace(/号/g, '#')) !== -1)
          if (x.length) {
            show = false
