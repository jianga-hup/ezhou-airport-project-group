...
const hadleclick = (e: imgss) => {
  ...
  if (e.type === 'fw') {
    global.$BlackHole3D.Camera.setCamLocateTo(mapuser.xmqfw, 0, 0)
  }
}
...
const onswitch = (key: string) => {
  if (key == 'tdt') {
    ...
    if (form[key]) {
      isLoadCAD(global) // 加载全部CAD
    } else {
      hideCAD(global) // 隐藏全部CAD
    }
    interactive.state.cads = interactive.state.cads.fill(form[key] ? true : false)
  } else if (key === 'hfyx') {
