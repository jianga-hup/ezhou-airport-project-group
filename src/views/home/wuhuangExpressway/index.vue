...
* @fileName index.vue
* @author
* @date 2024/05/14 11:57:00
!-->
<template>
  <div class="wuhuangExpressway">
    <TopCard :list="list" />
      ...
      <div :class="['left', { 'slide-in-left': isleft }]" v-if="isleft">
        <Overview
          :info="{
            mileage: '37.4',
            total: '69.09',
            category: '双向八车道高速公路改扩建工程',
            list: ['湖北交投建设集团有限公司', '中建三局集团有限公司']
          }"
        />
        <VideoMonitoring />
        <ProjectMilestones />
    ...
    <div :class="[isright ? 'overview-right' : 'overview2']">
      ...
      />
      <div :class="['right', { 'slide-in-right': isright }]" v-if="isright">
        <ProgressSatistics :value="3.04" />
        <QualityStatistics />
      </div>
    </div>
...
import BaseMap from './components/baseMap.vue' // 二维底图
import TopCard from '../easternHubei/content/projectOverviewTopCard.vue' // 头部卡片
import Overview from '../easternHubei/overviewLeft/overview.vue' // 项目概括
import VideoMonitoring from '../easternHubei/overviewLeft/videoMonitoring.vue' // 视频监控
import ProjectMilestones from '../easternHubei/overviewLeft/projectMilestones.vue' // 项目大事记
import ProgressSatistics from '../easternHubei/overviewRight/progressSatistics.vue' // 进度统计
import QualityStatistics from '../easternHubei/overviewRight/qualityStatistics.vue' // 进度统计
import SceneInformation from './components/sceneInformation.vue' // 场景信息
